...
class BaseNetwork():
    def __init__(self, a_size, scope, trainer):
        with tf.variable_scope(scope):
            ...
            self.pool1 = slim.max_pool2d(inputs=self.conv1, kernel_size=[2,2], stride=[1, 1])

            self.conv2 = slim.conv2d(inputs=self.pool1, num_outputs=32,
                                     kernel_size=[5, 5], stride=[1, 1], padding='SAME')

            self.pool2 = slim.max_pool2d(inputs=self.conv2, kernel_size=[2, 2], stride=[1, 1])

            self.conv3 = slim.conv2d(inputs=self.pool2, num_outputs=64,
                                     kernel_size=[4, 4], stride=[1, 1], padding='SAME')

            self.pool3 = slim.max_pool2d(inputs=self.conv3, kernel_size=[2, 2], stride=[1, 1])
            ...
            # self.deconv1 = slim.conv2d_transpose(inputs=rnn_out, num_outputs=32, kernel_size=[4, 4], stride=[1, 1])
            # self.deconv2 = slim.conv2d_transpose(inputs=self.deconv1, num_outputs=32, kernel_size=[8, 8], stride=[4, 4])
            #
            # b_deconv_v = tf.placeholder(tf.float32, [4, 4, 1, 32])
            # self.deconv_v = tf.nn.relu(self.deconv2 + b_deconv_v)
            # b_deconv_a = tf.placeholder(tf.float32, [4, 4, a_size, 32])
            # self.deconv_a = tf.nn.relu(self.deconv2 + b_deconv_a)
            #
            # self.pc_q = self.deconv_v + self.deconv_a - tf.reduce_mean(self.deconv_a, reduction_indices=3, keep_dims=True)
            # self.max_pixel_change = tf.reduce_max(self.pc_q, reduction_indices=3, keep_dims=False)
            ...
            if scope != 'global':
                ...
                # self.pc_a = tf.placeholder("float", [None, a_size])
                # pc_a_reshaped = tf.reshape(self.pc_a, [-1, 1, 1, a_size])
                ...
                self.loss = 0.5 * self.value_loss + self.policy_loss - self.entropy * 0.1 #+ tf.nn.l2_loss(self.pc_q - pc_qa)
            ...
            while not coord.should_stop():
                ...
                while d is False:
                    ...
                    a_dist, v, rnn_state = sess.run(
                        ...
                        feed_dict={self.local_AC.inputs: [s], # np.stack(np.array(episode_buffer)[-4:, 0])
                                   self.local_AC.state_in[0]: rnn_state[0], #np.array(episode_buffer)[-4, 6][0],
                                   self.local_AC.state_in[1]: rnn_state[1]})
                    ...
                    episode_buffer.append([s, a, r, s1, d, v[0, 0], rnn_state])
                    ...
                    if len(episode_buffer) == 10 and d != True and self.create_submission != True:
