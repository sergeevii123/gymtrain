...
class Node(object):
    ...
    def is_leaf(self):
        ...
        >>> Node().is_leaf
        True
        >>> Node( Variable(torch.Tensor([[1]])), left=Node(Variable(torch.Tensor([[2]]))) ).is_leaf
        False
        """
    ...
    def height(self):
        ...
        0
        >>> create([ Variable(torch.Tensor([[1, 2]])) ]).height()
        1
        >>> create([ Variable(torch.Tensor([[1, 2]])), Variable(torch.Tensor([[2, 3]])) ]).height()
        2
        """
...
class KDNode(Node):
    ...
    def add(self, point):
        ...
        while True:
            ...
            if point[0][current.axis].data[0] < current.data[0][current.axis].data[0]:
    ...
    def should_remove(self, point, node):
        ...
        if not (self.data == point).data.all():
            return False
    ...
    def axis_dist(self, point, axis):
        ...
        """
        return math.pow(self.data[0][axis].data[0] - point[0][axis].data[0], 2)
    ...
    def is_valid(self):
        ...
        if self.left and self.data[0][self.axis].data[0] < self.left.data[0][self.axis].data[0]:
            return False

        if self.right and self.data[0][self.axis].data[0] > self.right.data[0][self.axis].data[0]:
            return False
    ...
    def extreme_child(self, sel_func, axis):
        ...
        max_key = lambda child_parent: child_parent[0].data[0][axis].data[0]
...
def create(point_list=None, dimensions=None, axis=0, sel_axis=None):
    ...
    point_list.sort(key=lambda point: point[0][axis].data[0])
...
def check_dimensionality(point_list, dimensions=None):
    dimensions = dimensions or point_list[0].size()[1]
    for p in point_list:
        if p.size()[1] != dimensions:
